...
static int script_wait_event(lua_State *L)
{
    ...
    case MPV_EVENT_ERROR: {
        if (event->in_reply_to == 0)
            check_error(L, event->error);
        lua_pushinteger(L, event->in_reply_to); // event replyid
        lua_setfield(L, -2, "reply_id"); // event
        break;
    }
...
static int script_property_string(lua_State *L)
{
    ...
    if (err != MPV_ERROR_NOT_FOUND &&
        err != MPV_ERROR_PROPERTY &&
